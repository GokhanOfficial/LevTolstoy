...
router.post("/start", async (req, res) => {
	try {
		...
		processTask(task.id, files, model || "gemini-3.5-flash");
...
async function buildFileInputs(task, files) {
	...
	for (let i = 0; i < files.length; i++) {
		...
		const isS3 =
			file.storage === "s3" || (file.url && file.url.startsWith("/api/files/"));
		if (isS3 && file.url) {
	...
	try {
		...
		const markdown = await fileHandler.processMultipleFiles(
			...
			(chunk) => {
				...
				const elapsedAiSeconds = Math.max(
					(Date.now() - (task.aiStartedAt || Date.now())) / 1000,
					0,
				);
				const charProgress = Math.min(task.markdown.length / 10000, 1);
				const timeProgress = Math.min(elapsedAiSeconds / 60, 1);
				const blendedAiProgress = Math.max(
					charProgress * 0.55 + timeProgress * 0.45,
					task.aiProgress?.fraction || 0,
				);
				...
				const remainingByChars =
					task.markdown.length > 0
						? Math.max(
								0,
								((10000 - task.markdown.length) /
									Math.max(task.markdown.length, 1)) *
									elapsedAiSeconds,
							)
						: remainingByTime;
				...
				task.etaSeconds = Math.ceil(
					Math.min(
						Math.max(remainingByTime, 5),
						Math.max(remainingByChars, 5),
						60,
					),
				);
				task.aiProgress = {
			...
			{},
			{
				...
				onMediaProgress: (progress) => {
					...
					const elapsedPhaseSeconds = Math.max(
						(Date.now() - task.phaseStartedAt) / 1000,
						1,
					);
					const mediaEta =
						mediaPercent > 0
							? Math.ceil(
									(elapsedPhaseSeconds / mediaPercent) * (100 - mediaPercent),
								)
							: null;
		...
		console.error(
			`${wasCancelled ? "🛑" : "❌"} Task ${wasCancelled ? "iptal" : "hata"} (${taskId}):`,
			error.message,
		);
		updateTask(task, {
