...
---`;
  ...
  return decision === 'summarize' ? 'summarize' : 'convert';
}

// ── Text MIME types that can be used as-is without AI ──────────────────────
const TEXT_MIMES = new Set(['text/markdown', 'text/html', 'text/plain', 'text/x-markdown']);

/**
 * Returns true when ALL files in the task are plain text types (MD/HTML/TXT)
 * and the task type is 'convert' — meaning we can skip the AI entirely and
 * render the content directly to PDF/HTML/MD.
 */
function isDirectRender(task) {
  if (task.type === 'summarize' || task.type === 'auto') return false;
  return task.files.every(f => TEXT_MIMES.has(f.mimetype));
}
...
async function processTask(task, queue) {
  ...
  try {
    ...
    // ── Fast path: MD/HTML/TXT → render directly, no AI ─────────────────────
    if (isDirectRender(task)) {
      update({ phase: 'rendering', message: 'İçerik hazırlanıyor…', progress: 20 });

      const parts = task.files.map(f => {
        const text = f.buffer.toString('utf8');
        // If HTML, strip tags to get plain text that marked can re-render cleanly;
        // for markdown just use as-is.
        if (f.mimetype === 'text/html') {
          // Keep raw HTML — marked can parse it or we pass straight to htmlRenderer
          return text;
        }
        return text;
      });
      ...
      if (actualType === 'summarize') {
        // Convert to markdown first, then summarize
        const rawMarkdown = await fileHandler.processMultipleFiles(fileInputs, task.model, (chunk) => {
          markdown += chunk;
          update({ progress: Math.min(80, 25 + Math.floor((markdown.length / 10000) * 55)) });
        }, {}, {
          ...fileProcessOptions,
          onAiStart: () => {
        ...
        update({ phase: 'ai-conversion', message: 'AI ile özetleniyor...', progress: 60 });
        const summary = await openai.summarizeText(markdown, task.model, () => {
    ...
    // 3. Generate filename
    if (isDirectRender(task)) {
      // For direct-render tasks, extract title from H1 or use original filename
      const h1 = markdown.match(/^#\s+(.+)$/m);
      if (h1) {
        task.filename = h1[1].trim()
          .replace(/\s+/g, '-')
        ...
        const orig = task.files[0]?.filename || 'document';
        task.filename = orig.replace(/\.[^.]+$/, '') || 'document';
      }
    } else {
      try {
