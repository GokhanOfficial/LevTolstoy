...
function HistoryCard({ entry }: { entry: HistoryEntry }) {
  ...
  return (
    <div className="group rounded-xl border border-slate-800 bg-slate-900/50 overflow-hidden transition-all">
      {/* Header */}
      <div className="flex items-start justify-between gap-3 px-4 py-3">
        <div className="min-w-0 flex-1">
          <p className="truncate text-sm font-medium text-slate-200 leading-tight">{entry.title}</p>
          <p className="mt-0.5 text-[11px] text-slate-600">{formatDate(entry.createdAt)}</p>
        </div>
        <div className="flex items-center gap-1 shrink-0">
          <button
            onClick={() => openInEditor(entry.markdown, entry.title)}
            title="Editörde düzenle"
            className="rounded p-1.5 text-slate-600 transition-colors hover:bg-indigo-500/10 hover:text-indigo-400"
          >
          ...
          <button
            ...
            title="Sil"
            className="rounded p-1.5 text-slate-600 transition-colors hover:bg-red-400/10 hover:text-red-400"
          >
          ...
          <button
            ...
            className="rounded p-1.5 text-slate-600 transition-colors hover:text-slate-300"
          >
      ...
      {!expanded && (
        <p className="px-4 pb-3 text-xs text-slate-600 leading-relaxed line-clamp-2">{excerpt}</p>
      )}

      {/* Expanded: download buttons */}
      {expanded && (
        <div className="border-t border-slate-800 bg-slate-900/30 px-4 py-3 space-y-2">
          <p className="text-[11px] text-slate-600 mb-2">İndir:</p>
          <div className="flex gap-2">
            <button
              onClick={() => downloadBlob(entry.markdown, `${base}.md`, 'text/markdown')}
              className="flex flex-1 items-center justify-center gap-1.5 rounded-lg border border-sky-500/30 bg-sky-500/10 px-2 py-1.5 text-xs font-semibold text-sky-400 transition-colors hover:bg-sky-500/20"
            >
            ...
            <button
              ...
              className="flex flex-1 items-center justify-center gap-1.5 rounded-lg border border-orange-500/30 bg-orange-500/10 px-2 py-1.5 text-xs font-semibold text-orange-400 transition-colors hover:bg-orange-500/20"
            >
              <Download className="size-3.5" /> HTML
            </button>
            <button
              onClick={() => printAsPdf(entry.markdown, base)}
              className="flex flex-1 items-center justify-center gap-1.5 rounded-lg border border-red-500/30 bg-red-500/10 px-2 py-1.5 text-xs font-semibold text-red-400 transition-colors hover:bg-red-500/20"
            >
    ...
    <section className="mx-auto max-w-2xl space-y-4">
      <div className="flex items-center justify-between">
        ...
        >
          ...
          Geçmiş
          <span className="rounded-full bg-slate-800 px-2 py-0.5 text-[11px] font-medium text-slate-500">
            {entries.length}
          </span>
          {collapsed ? <ChevronDown className="size-3.5 text-slate-600" /> : <ChevronUp className="size-3.5 text-slate-600" />}
        </button>
        <button
          onClick={clear}
          className="text-xs text-slate-700 transition-colors hover:text-red-400"
        >
          Tümünü sil
        </button>
      </div>
