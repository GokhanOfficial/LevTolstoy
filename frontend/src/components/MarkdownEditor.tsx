...
export default function MarkdownEditor() {
  ...
  return (
    <div className="flex h-[calc(100vh-56px)] flex-col bg-background transition-colors duration-200">

      {/* ── Toolbar ──────────────────────────────────────────────── */}
      <div className="flex flex-wrap items-center gap-1 border-b border-border bg-card/80 px-3 py-2">
        {/* File name */}
        <input
          value={title}
          onChange={(e) => dispatch({ type: 'SET_TITLE', title: e.target.value })}
          className="mr-2 w-40 rounded border border-border bg-background px-2 py-1 text-xs text-foreground outline-none focus:border-indigo-500/60"
        ...
        {TOOLBAR.map((item, i) => (
          <span key={i} className="flex items-center">
            {item.sep && <span className="mx-1 h-4 w-px bg-border" />}
            <button
        ...
        <button onClick={() => downloadBlob(content, `${baseName}.md`, 'text/markdown')}
          className="flex h-7 items-center gap-1.5 rounded border border-sky-500/30 bg-sky-500/10 px-2 text-xs font-medium text-sky-500 transition-colors hover:bg-sky-500/20">
        ...
        <button onClick={() => {
          ...
          className="flex h-7 items-center gap-1.5 rounded border border-orange-500/30 bg-orange-500/10 px-2 text-xs font-medium text-orange-500 transition-colors hover:bg-orange-500/20">
          <Download className="size-3.5" /> HTML
        </button>
        <button onClick={() => printAsPdf(content, baseName)}
          className="flex h-7 items-center gap-1.5 rounded border border-red-500/30 bg-red-500/10 px-2 text-xs font-medium text-red-500 transition-colors hover:bg-red-500/20">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {(viewMode === 'edit' || viewMode === 'split') && (
          <div className={cn(
            ...
            <div className="flex flex-1 overflow-hidden font-mono text-sm">
              ...
              <div
                ref={lineNumsRef}
                className="select-none overflow-hidden border-r border-border bg-muted/30 px-3 pt-4 text-right text-xs leading-6 text-muted-foreground/50"
                style={{ minWidth: '3rem' }}
              >
              ...
              <textarea
                ...
                spellCheck={false}
                className="flex-1 resize-none bg-background px-4 pt-4 pb-16 text-foreground outline-none leading-6 placeholder:text-muted-foreground/40"
        ...
        {(viewMode === 'preview' || viewMode === 'split') && (
          <div className={cn(
            'flex-1 overflow-y-auto bg-background px-8 py-6',
            viewMode === 'split' ? 'w-1/2' : 'w-full'
          )}>
            <div className="mx-auto max-w-3xl prose max-w-none">
      ...
      {/* ── Status bar ──────────────────────────────────────────────── */}
      <div className="flex items-center gap-4 border-t border-border bg-card/60 px-4 py-1.5 text-[11px] text-muted-foreground">
