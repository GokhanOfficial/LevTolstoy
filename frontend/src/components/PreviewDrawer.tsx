...
export default function PreviewDrawer() {
  ...
  return (
    <>
      ...
      {/* Drawer panel */}
      <div className="fixed inset-y-0 right-0 z-[101] flex w-full max-w-2xl flex-col bg-slate-900 shadow-2xl border-l border-slate-800">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-slate-800 px-5 py-3.5">
          <div className="min-w-0 flex-1">
            <p className="truncate text-sm font-semibold text-slate-200">{filename}</p>
            ...
            {previewTaskId && (
              <>
                <button
                  ...
                  className="flex items-center gap-1.5 rounded-lg border border-slate-700 px-3 py-1.5 text-xs font-medium text-slate-400 hover:border-indigo-500/50 hover:text-indigo-400 transition-colors"
                >
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 rounded-lg border border-slate-700 px-3 py-1.5 text-xs font-medium text-slate-400 hover:border-red-500/50 hover:text-red-400 transition-colors"
                >
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          {!state.loading && !state.content && (
            <div className="flex items-center justify-center h-full">
              <p className="text-sm text-slate-500">İçerik yüklenemedi.</p>
            </div>
          )}
