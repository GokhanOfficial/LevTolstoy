...
function updateConversionProgress(status) {
	...
	if (progressTitle)
		progressTitle.textContent = status.message || "Dönüştürülüyor...";
	if (progressFilename) progressFilename.textContent = status.currentFile || "";
	if (progressPhase)
		progressPhase.textContent = formatProgressPhase(
			status.phase || status.status,
		);
	if (progressPercent)
		progressPercent.textContent = `${progress}% • kalan ${formatProgressDuration(status.etaSeconds)}`;

	if (progressDetail) {
		const media = status.mediaProgress;
		const ai = status.aiProgress;
		if (media) {
			const bitrate = media.bitrateKbps
				? `Bitrate: ${media.bitrateKbps}kbps`
				: "Bitrate hesaplanıyor";
			const attempt = media.attempt ? `Deneme: ${media.attempt}` : "";
			const duration = media.duration
				? `Süre: ${formatProgressDuration(media.duration)}`
				: "";
			const elapsed = media.elapsedSeconds
				? `Geçen: ${formatProgressDuration(media.elapsedSeconds)}`
				: "";
			...
			const chars =
				typeof ai.chars === "number" ? ai.chars.toLocaleString("tr-TR") : "0";
			const expected =
				typeof ai.expectedChars === "number"
					? ai.expectedChars.toLocaleString("tr-TR")
					: "10.000";
			progressDetail.innerHTML = `
...
const HomePage = {
	mount() {
		...
		// View tabs
		document
			.getElementById("tab-markdown")
			?.addEventListener("click", () => window.app.setView("markdown")); // Assuming app.js exposes setView
		document
			.getElementById("tab-preview")
			?.addEventListener("click", () => window.app.setView("preview"));
		document
			.getElementById("tab-split")
			?.addEventListener("click", () => window.app.setView("split"));
	...
	initButtons() {
		...
		// Download MD button
		document
			.getElementById("download-md-btn")
	...
	async handleConvert() {
		...
		if (cachedFiles.length === 0) {
			window.utils.showToast(
				window.i18n?.t("toast.noFiles") || "Please select files first",
				"error",
			);
			return;
		}
		...
		if (window.fileUpload?.isUploading()) {
			window.utils.showToast(
				window.i18n?.t("toast.uploadingWait") ||
		...
		if (progressFilename) {
			progressFilename.textContent =
				cachedFiles.length === 1
					? cachedFiles[0].filename
					: `${cachedFiles.length} dosya`;
		}
		...
		try {
			...
			cachedFiles.forEach((_, i) =>
				window.fileUpload.updateFileStatus(i, "converting"),
			);
			...
			const startResult = await window.api.startConversion(
				cachedFiles,
				selectedModel,
			);
			...
			while (!completed) {
				...
				if (status.status === "completed") {
					completed = true;
					window.app.currentMarkdown = status.markdown;
					window.app.currentFilename =
						cachedFiles.length === 1 ? cachedFiles[0].filename : "combined";
					...
					// Update all file statuses
					cachedFiles.forEach((_, i) =>
						window.fileUpload.updateFileStatus(i, "done"),
					);
					window.utils.showToast(
						window.i18n?.t("toast.conversionComplete") ||
							"Conversion complete!",
						"success",
					);
					...
					completed = true;
					cachedFiles.forEach((_, i) =>
						window.fileUpload.updateFileStatus(i, "cancelled"),
					);
			...
			cachedFiles.forEach((_, i) =>
				window.fileUpload.updateFileStatus(
					i,
					cancelRequested ? "cancelled" : "error",
				),
			);
			...
			window.utils.showToast(
				cancelRequested
					...
					: error.message ||
							window.i18n?.t("toast.conversionFailed") ||
							"Conversion failed",
				cancelRequested ? "info" : "error",
			);
			if (progressSection) progressSection.classList.add("hidden");
			...
			if (isMissingFile) {
				window.utils.showToast(
					"Dosya sunucuda bulunamadı (zaman aşımı). Listeden kaldırıldı, lütfen tekrar yükleyin.",
					"warning",
				);
